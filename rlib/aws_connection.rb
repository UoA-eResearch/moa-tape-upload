...
class AWS_connection
  ...
  def obj_md5(key:)
    ...
    rescue Aws::S3::Errors::NotFound => _e
      return nil
    rescue   StandardError => e
      $stderr.puts "obj_md5(#{@bucket}) : #{e.class} #{e}"
    end
  end
    ...
    begin
      if not_if_exits
        begin
          ...
          $stderr.puts "AWS_connection(#{@bucket}).object_put_mem(#{key}) MD5: #{e.class} #{e}"
        end
      end
      _resp = @s3_client.put_object(
        key: key,
        body: content,
        bucket: @bucket,
      ...
      $stderr.puts "AWS_connection(#{@bucket}).object_put_mem(#{key}): #{e.class} #{e}"
    end
  end
  ...
  def object_get_mem(key:)
    begin
      ...
      puts "object_get_mem(#{@bucket}, #{key}) : #{e.class} #{e}"
    end
  end
  ...
  def object_get_file(key:, filename:, range: nil)
    if File.exist?(filename)
      ...
      if stat_record.directory?
        filename = "#{filename.gsub(/\/$/, '')}/#{File.basename(key)}"
      end
    end

    begin
      _resp = @s3_client.get_object( response_target: filename, bucket: @bucket, key: key, range: nil)
    rescue StandardError => e
      puts "object_get_file(#{@bucket},#{key},#{filename},#{range}) : #{e.class} #{e}"
    end
  end
  ...
  def bucket_ls(prefix: nil, delimiter: nil, versions: false, &block)
    begin
      size = 0
      count = 0
      next_marker = nil
      ...
      begin
        ...
        if block_given?
          objects.common_prefixes.each do |com_prefix|
            yield OpenStruct.new(bucket: bucket, key: com_prefix.prefix, last_modified: Time.now,
                                 owner: OpenStruct.new(display_name: '', id: ''), size: 0, storage_class: ''
            )
          end
          if versions
            objects.versions.each(&block)
          else
            objects.contents.each(&block)
          end
        else
          objects.common_prefixes.each { |com_prefix| puts com_prefix.prefix }
          if versions
      ...
      puts "bucket_ls(#{@bucket}) : #{e.class} #{e}"
    end
  end
  ...
  def bucket_get_acl
    begin
      ...
      puts "bucket_get_acl(#{@bucket}) : #{e.class} #{e}"
    end
  end
  ...
  def object_get_acl(key:)
    begin
      ...
      puts "object_get_acl(#{@bucket},#{key}) : #{e.class} #{e}"
    end
  end
  ...
  def object_copy(src_key:, dest_bucket:, dest_key:, src_bucket: nil)
    begin
      src_bucket ||= @bucket
      @s3_client.copy_object( copy_source: "/#{src_bucket}/#{src_key}", bucket: dest_bucket, key: dest_key, metadata_directive: 'COPY')
    rescue StandardError => e
      puts "object_copy(#{src_bucket},#{src_key},#{dest_bucket},#{dest_key}) : #{e.class} #{e}"
    end
  end
  ...
  def rsync(dest_bucket:, src_bucket: nil)
    src_bucket ||= @bucket
    dest_dir_list = {}
  ...
  def bucket_stats(prefix: nil, delimiter: nil)
    begin
      ...
      puts "bucket_size(#{@bucket}) : #{e.class} #{e}"
    end
    return count, size
  end
  ...
  def object_metadata(key:)
    begin
      ...
      puts "object_metadata(#{@bucket}, #{key}): #{e.class} #{e}"
    end
  end

  def object_delete(key:, version_id: nil)
    begin
      _resp = @s3_client.delete_object( bucket: @bucket, key: key, version_id: version_id)
    rescue StandardError => e
      puts "object_delete(bucket: #{@bucket}, key: #{key}, version_id: #{version_id}) : #{e}"
    end
  end
end
