...
def object_put_dir(aws_fd:, directory:, strip_base_dir: nil, misc_prefix: 'misc', metadata_log: nil )
  ...
  begin
        ...
        begin
          if stat_record.symlink? == false # Otherwise ignore
            if stat_record.directory?
              ...
              rescue S3_Exists => _e # Get this if the file existed in the object store with the same MD5 sum.
                # puts e
              rescue StandardError => e
                $stderr.puts "Copy of \"#{qualified_filename}\" to object store failed: #{e}"
              end
            end
          end
        end
    ...
    $stderr.puts "object_put_dir(#{directory}) : #{e}"
  end
